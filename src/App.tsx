    ...
    <div ref={cardRef} className="scroll-mt-24 space-y-5">
      ...
      <div className="sticky bottom-[5rem] z-10 -mx-5 border-t border-slate-100 bg-white/95 px-5 py-3 backdrop-blur sm:static sm:mx-0 sm:border-t-0 sm:bg-transparent sm:p-0 sm:backdrop-blur-none">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
          {submitted && (
            <span className={`inline-flex items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold sm:mr-auto ${isCorrect ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>{isCorrect ? <CheckCircle2 className="h-5 w-5" /> : <XCircle className="h-5 w-5" />}{isCorrect ? '回答正确' : '回答错误'}</span>
          )}
          <button
            type="button"
            className="rounded-2xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300 sm:min-w-32"
            disabled={!submitted && selected.length === 0}
            onClick={() => submitted ? onNext() : void onSubmit()}
          >
            {submitted ? '下一题' : '提交答案'}
          </button>
        </div>
